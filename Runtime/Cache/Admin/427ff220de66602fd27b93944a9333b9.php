		...
		<table>
			...
			<tbody>
				<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$channel): $mod = ($i % 2 );++$i;?><tr>
						<!--<td><input class="ids" type="checkbox" name="ids[]" value="<?php echo ($channel["id"]); ?>" /></td>-->
                        ...
                        <td><?php echo ($channel['type']==1?"出租":"出售"); ?></td>
                        <td><?php echo ($channel["price"]); echo ($channel['company']==1?"元/月":"万元"); ?></td>
						<td><?php echo (date("Y-m-d H:i:s",$channel["create_time"])); ?></td>
						<td><?php echo ($channel["end_time"]); ?></td>
						<td><?php echo ($channel['status']==1?"启动":"禁用"); ?></td>
						<td>
					...
					<?php echo ($page); endforeach; endif; else: echo "" ;endif; ?>
				<?php else: ?>
